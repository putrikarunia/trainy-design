...
export function useDndCanvas({
  ...
  const dragStartStateRef = useRef<Map<string, {
  ...
  const getDraggedIds = (draggedId: string): Set<string> => {
  ...
  const handleDragStart = (event: DragStartEvent) => {
    const clickedElementId = event.active.id as string;
  ...
  const draggedIds = activeId ? getDraggedIds(activeId) : new Set<string>();
  const activeElements: FEElement[] = [];
  for (const id of draggedIds) {
    const el = findElement(elements, id);
