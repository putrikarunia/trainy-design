...
export async function uploadAsset(
  ...
  try {
    ...
    const result: UploadAssetResponse = await response.json();
 ...
 */
export async function fileToUrl(file: File, devServerUrl?: string): Promise<ImageResult> {
 ...
 */
export function createImageElement(src: string, position?: { x: number; y: number }): HtmlElement {
  return {
 ...
 */
export async function getImageFromDropEvent(e: React.DragEvent, devServerUrl?: string): Promise<ImageResult> {
 ...
 */
export async function copyElementToClipboard(element: FEElement): Promise<boolean> {
 ...
 */
export async function readElementFromClipboard(devServerUrl?: string): Promise<ClipboardReadResult> {
  try {
  ...
  return { element: clipboardData.element as FEElement };
}
 ...
 * If devServerUrl is provided, uploads pasted images to the server
 */
export async function pasteElementIntoTree(
  elements: FEElement[],
