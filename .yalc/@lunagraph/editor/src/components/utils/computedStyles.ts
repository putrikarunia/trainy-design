 ...
 */
function combineShorthands(
  styles: ComputedStyleEntry[],
  computed: CSSStyleDeclaration
): ComputedStyleEntry[] {
  const styleMap = new Map(styles.map(s => [s.property, s]))
  const result: ComputedStyleEntry[] = []
  const processed = new Set<string>()

  for (const style of styles) {
    if (processed.has(style.property)) continue
    ...
    if (style.property.startsWith('padding') && style.property !== 'padding') {
      ...
      result.push({
        ...
        isInlineStyle: (top as ComputedStyleEntry).isInlineStyle || (right as ComputedStyleEntry).isInlineStyle ||
                      (bottom as ComputedStyleEntry).isInlineStyle || (left as ComputedStyleEntry).isInlineStyle
      })
    ...
    if (style.property === 'rowGap' || style.property === 'columnGap') {
      ...
      if (rowGap.value === columnGap.value) {
        result.push({
          ...
          isInlineStyle: (rowGap as ComputedStyleEntry).isInlineStyle || (columnGap as ComputedStyleEntry).isInlineStyle
        })
        ...
        if (!processed.has('rowGap')) {
          result.push(rowGap as ComputedStyleEntry)
          processed.add('rowGap')
        }
        if (!processed.has('columnGap')) {
          result.push(columnGap as ComputedStyleEntry)
          processed.add('columnGap')
        }
      }
      continue
 ...
 */
function createShorthand(property: string, top: string, right: string, bottom: string, left: string): string {
  // All same: "24px"
...
export interface CascadeSource {
  selector: string // e.g., "p-4", "container", "inline"
  value: string // The CSS value from this source
  type: 'class' | 'inline' // class = from className, inline = from style attribute
  isActive: boolean // true if this source's value matches the final computed value
}

export interface ComputedStyleEntry {
  property: string // camelCase
  displayProperty: string // kebab-case
  value: string
  isUserOverride: boolean // true if user edited it (differs from className)
  isInlineStyle: boolean // true if exists in inline styles (can be deleted)
  cascade?: CascadeSource[] // cascade sources (lazy-loaded on expand)
}
 ...
 */
export function getCascadeForProperty(
  element: HTMLElement,
  property: string,
...
export function extractComputedStyles(
  element: HTMLElement,
  userOverrides: React.CSSProperties = {}
): ComputedStyleEntry[] {
  ...
  const styles: ComputedStyleEntry[] = []
  ...
  for (let i = 0; i < computed.length; i++) {
    ...
    const isNoisyAuto = !isUserOverride && (
      actualValue === 'auto' ||
      actualValue === 'normal' ||
      actualValue === 'none' ||
      actualValue === 'static' ||
