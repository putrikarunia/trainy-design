...
export function useHoverAndSelectionOverlay({ canvasRef, elements, selectedElementIds, hoverElementId, handleResizeStart, transform, onSelectElement, onHoverElement, onUpdateElementStyles, onUpdateElementProps, readOnly = false, }) {
    const [elementRects, setElementRects] = useState(new Map());
    const updateRectsRef = useRef(null);
    ...
    const updateRects = useCallback(() => {
        ...
        const relevantIds = new Set([...selectedElementIds]);
        if (hoverElementId)
            relevantIds.add(hoverElementId);
    ...
    const renderSelectionOverlay = () => {
        ...
        for (const selectedElementId of selectedElementIds) {
            ...
            overlays.push(_jsxs("div", { className: cn("absolute pointer-events-none outline outline-2", isComponent ? "outline-purple-500" : "outline-blue-500"), style: {
                    left: relativeRect.left,
                    top: relativeRect.top,
                    width: relativeRect.width,
                    height: relativeRect.height,
                }, children: [isSingleSelection && (_jsxs("div", { className: cn("absolute -top-5 flex items-center gap-1 select-none", !readOnly && "pointer-events-auto"), onMouseDown: (e) => handleLabelMouseDown(e, selectedElementId), children: [_jsx(Text, { size: "xs", className: cn(isComponent ? "text-purple-500" : "text-blue-500"), children: selectedElement.type === 'html' ? selectedElement.tag :
                                    selectedElement.type === 'icon' ? selectedElement.iconName :
                                        selectedElement.type === 'component' ? selectedElement.componentName : 'Element' }), selectedElement.type === 'html' && selectedElement.tag === 'iframe' && !readOnly && (_jsxs(Tooltip, { children: [_jsx(TooltipTrigger, { asChild: true, children: _jsx("button", { className: cn("flex items-center gap-0.5 px-1 py-0.5 rounded text-[10px] font-medium transition-colors cursor-pointer", selectedElement.props?.['data-interaction-enabled'] !== 'true'
                                                ? "bg-ed-muted text-ed-muted-foreground hover:bg-ed-accent"
                                                : "bg-blue-500 text-white hover:bg-blue-600"), onClick: (e) => {
                                                e.stopPropagation();
                                                e.preventDefault();
                                            ...
                                            }, onMouseDown: (e) => e.stopPropagation(), children: _jsx(CursorClick, { size: 10, weight: "bold" }) }) }), _jsx(TooltipContent, { side: "top", children: selectedElement.props?.['data-interaction-enabled'] !== 'true'
                                            ? "Interaction disabled. Click to enable."
                                            : "Interaction enabled. Click to disable." })] }))] })), isSingleSelection && (_jsxs(_Fragment, { children: [_jsx("div", { className: cn("absolute w-2 h-2 bg-white border cursor-nw-resize pointer-events-auto", isComponent ? "border-purple-500" : "border-blue-500"), style: { left: -4, top: -4 }, onMouseDown: (e) => handleResizeStart(e, selectedElementId, "nw", selectedElement) }), _jsx("div", { className: cn("absolute w-2 h-2 bg-white border cursor-ne-resize pointer-events-auto", isComponent ? "border-purple-500" : "border-blue-500"), style: { right: -4, top: -4 }, onMouseDown: (e) => handleResizeStart(e, selectedElementId, "ne", selectedElement) }), _jsx("div", { className: cn("absolute w-2 h-2 bg-white border cursor-sw-resize pointer-events-auto", isComponent ? "border-purple-500" : "border-blue-500"), style: { left: -4, bottom: -4 }, onMouseDown: (e) => handleResizeStart(e, selectedElementId, "sw", selectedElement) }), _jsx("div", { className: cn("absolute w-2 h-2 bg-white border cursor-se-resize pointer-events-auto", isComponent ? "border-purple-500" : "border-blue-500"), style: { right: -4, bottom: -4 }, onMouseDown: (e) => handleResizeStart(e, selectedElementId, "se", selectedElement) })] }))] }, selectedElementId));
        }
        return _jsx(_Fragment, { children: overlays });
    };
    // Handle mousedown on label to trigger drag on the actual element
    const handleLabelMouseDown = useCallback((e, elementId) => {
        if (readOnly)
            return;
    ...
    const handleLabelClick = useCallback((e, elementId) => {
        if (readOnly)
            return;
        e.stopPropagation();
        onSelectElement?.(elementId, e.shiftKey);
    ...
    const handleLabelMouseEnter = useCallback((elementId) => {
        onHoverElement?.(elementId);
    }, [onHoverElement]);
    const renderHoverOverlay = () => {
        if (!hoverElementId)
            return null;
        if (selectedElementIds.has(hoverElementId))
            return null;
        ...
        return (_jsx("div", { className: cn("absolute pointer-events-none outline outline-1", isComponent ? "outline-purple-500" : "outline-blue-500"), style: {
                left: relativeRect.left,
                top: relativeRect.top,
                width: relativeRect.width,
                height: relativeRect.height,
            }, children: _jsx("div", { className: cn("absolute -top-5 flex items-center select-none", !readOnly && "pointer-events-auto"), onClick: (e) => handleLabelClick(e, hoverElementId), onMouseDown: (e) => handleLabelMouseDown(e, hoverElementId), onMouseEnter: () => handleLabelMouseEnter(hoverElementId), children: _jsx(Text, { size: "xs", className: cn(isComponent ? "text-purple-500" : "text-blue-500"), children: labelText }) }) }));
    };
    return {
