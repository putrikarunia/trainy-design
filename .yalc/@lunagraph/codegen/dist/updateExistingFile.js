 ...
 */
export function updateExistingFile(options) {
 ...
 */
function replaceReturnStatement(path, newJSXNode) {
 ...
 */
function updateImports(ast, newDependencies, componentIndex, targetFilePath) {
    ...
    const componentPaths = new Set(Object.values(componentIndex).map(entry => entry.path.replace(/\.tsx?$/, '')));
    const existingImportIndices = [];
 ...
 */
function buildJSXElementsString(elements) {
...
function buildJSXElement(element) {
