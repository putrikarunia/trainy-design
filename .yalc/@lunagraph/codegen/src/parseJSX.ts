    ...
    const walk = (node: any) => {
      ...
      if (t.isJSXFragment(node)) {
        ...
        const children = node.children
          .map((child: any) => parseJSXChild(child, iconLibraries, components, defaultIconLibrary))
...
function parseJSXElement(
  ...
  if (props && props['data-icon']) {
    ...
    const iconStyles: Record<string, any> = styles ? { ...styles } : {}
