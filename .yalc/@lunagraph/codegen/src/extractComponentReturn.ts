...
/**
 * Candidate component info (without storing path - paths become stale after traversal)
 */
 ...
 */
export function extractComponentReturn(code: string, options: ExtractComponentOptions = {}): ExtractedComponent | null {
  try {
    ...
    const moduleConstantDeclarations: string[] = []
    const moduleFunctionDeclarations: string[] = []
    ...
    let result: { returnJSX: string; conditionalReturns?: ConditionalReturn[] } | null = null
    ...
    traverse(ast, {
      ...
      VariableDeclaration(path: any) {
        ...
        for (let i = 0; i < path.node.declarations.length; i++) {
          const declarator = path.node.declarations[i]
          if (!t.isIdentifier(declarator.id)) continue
          if (declarator.id.name !== selectedName) continue
          ...
          if (t.isCallExpression(init) && isForwardRefCall(init)) {
            const callbackArg = init.arguments[0] as t.Expression
            if (t.isArrowFunctionExpression(callbackArg) || t.isFunctionExpression(callbackArg) || t.isTSAsExpression(callbackArg)) {
              ...
              if (t.isTSAsExpression(finalCallback)) {
                finalCallback = finalCallback.expression as any
              }
              if (!t.isArrowFunctionExpression(finalCallback) && !t.isFunctionExpression(finalCallback)) {
                return
              }
              propsType = extractPropsType((finalCallback as any).params)
              const callbackPath = path.get(`declarations.${i}.init`) as any
    ...
    const siblingExportedComponents = Array.from(exportedNames).filter(name =>
      name !== componentName && /^[A-Z]/.test(name)
    )
 ...
 */
function extractImports(ast: any, imports: string[], importedNames: Set<string>) {
  traverse(ast, {
 ...
 */
function extractTypeDeclarations(ast: any, typeDeclarations: string[]) {
  traverse(ast, {
 ...
 */
function extractModuleFunctionDeclarations(ast: any, declarations: string[], targetComponentName?: string) {
      ...
      for (const statement of programPath.node.body) {
        ...
        if (t.isFunctionDeclaration(statement)) {
          if (statement.id && t.isIdentifier(statement.id)) {
            ...
            if (name === targetComponentName) continue
 ...
 */
function extractInternalComponentProps(params: any[]): Record<string, { type: string; required: boolean }> | undefined {
        ...
        for (const member of typeAnnotation.members) {
          if (t.isTSPropertySignature(member) && t.isIdentifier(member.key)) {
            ...
            if (props[propName]) {
              ...
              if (isOptional) props[propName].required = false
            } else {
 ...
 */
function extractInternalComponents(
  ast: any,
  internalComponents: InternalComponent[],
 ...
 */
function extractPropsType(params: any[]): string | null {
...
function extractFromFunction(
  ...
  variableDeclarations: string[],
  hookCalls: HookCall[]
): { returnJSX: string; conditionalReturns?: ConditionalReturn[] } | null {
  ...
  const variablesWithMethodCalls = new Set<string>()
 ...
 */
function findConditionalReturns(path: any): ConditionalReturn[] | null {
  const returns: { condition: string | null; jsx: string; node: any }[] = []
      ...
      while (currentPath && currentPath !== path) {
        if (t.isIfStatement(currentPath.node)) {
          ...
          const isInConsequent = t.isBlockStatement(consequent)
            ? consequent.body.some((stmt: any) => containsNode(stmt, returnPath.node))
 ...
 */
function containsNode(parent: any, target: any): boolean {
  if (parent === target) return true

  if (t.isBlockStatement(parent)) {
    return parent.body.some((stmt: any) => containsNode(stmt, target))
  }
 ...
 */
function findVariablesWithMethodCalls(jsxNode: any): Set<string> {
  const result = new Set<string>()
  if (!jsxNode) return result
 ...
 */
function extractHookCalls(path: any, hookCalls: HookCall[], initialValues: Record<string, any>, variablesWithMethodCalls: Set<string>) {
  path.traverse({
    ...
    ExpressionStatement(stmtPath: any) {
...
function findVariablesInJSX(node: any, variables: Set<string>) {
  ...
  function findIdentifiersInExpression(expr: any, parent: any = null) {
    if (!expr) return
  ...
  function findVariablesInJSXNode(jsxNode: any) {
    if (!jsxNode) return
...
function extractPropsFromParams(params: any[], props: Set<string>, propDefaults: Record<string, string>) {
...
function extractInitialValues(
  ...
  const allDeclarations: { code: string; varNames: string[]; initNodes: any[] }[] = []
  const allFunctionDecls: { code: string; name: string }[] = []
  ...
  const neededFunctionDecls = new Set<string>()
  let changed = true
  ...
  while (changed) {
    ...
    for (const decl of allDeclarations) {
      ...
      if (neededDeclarations.has(decl.code)) continue

      // Check if any declared variable is in our variables set
      const hasRelevantVariable = decl.varNames.some(
        varName => variables.has(varName) && !props.has(varName)
      )
      ...
      if (hasRelevantVariable) {
        ...
        for (const initNode of decl.initNodes) {
          traverse(
            t.file(t.program([t.expressionStatement(initNode)])),
            {
    ...
    for (const funcDecl of allFunctionDecls) {
      if (neededFunctionDecls.has(funcDecl.code)) continue
 ...
 */
function extractModuleLevelConstants(
  ast: any,
  moduleConstants: Record<string, any>,
  moduleConstantDeclarations: string[]
) {
 ...
 */
function evaluateModuleConstant(node: any): any {
  ...
  try {
    ...
    if (t.isArrayExpression(node)) {
      return node.elements.map((el: any) => evaluateModuleConstant(el))
    }
