import { ExtractedComponent, HookCall } from './extractComponentReturn.js'
...
export function generateSnapshot(
  ...
  extractedTypes?: ExtractedTypes | null,
  options?: SnapshotGenerationOptions
): SnapshotGenerationResult {
  const snapshotProps: SnapshotProp[] = []
  const lines: string[] = []
  ...
  const propTypeMap = new Map<string, ExtractedPropType>()
  const stateTypeMap = new Map<string, ExtractedPropType>()
  const constantTypeMap = new Map<string, ExtractedPropType>()
  ...
  const hasReactNamespaceImport = deconflictedImports.some(imp =>
    imp.includes('* as React') || imp.match(/import\s+React\s+from/)
  )
  ...
  const filteredComponentsToImport = componentsToImport.filter(
    name => !localComponentNames.has(name)
  )
  ...
  for (const decl of extracted.variableDeclarations) {
    ...
    if (snapshotPropNames.has(varName)) continue

    // Skip event handlers
    if (/^(handle|on)[A-Z]/.test(varName)) continue
    ...
    if (initialValue !== undefined && (Array.isArray(initialValue) || (typeof initialValue === 'object' && initialValue !== null))) {
      ...
      if (!Array.isArray(initialValue)) {
        ...
        const isConfigObject = values.length <= 6 && values.every(v => 
          typeof v === 'number' || typeof v === 'string' || typeof v === 'boolean'
        )
        if (isConfigObject) continue
      }
...
function resolveImportPaths(
  ...
  return imports.map(importStr => {
    ...
    const resolvedLines = lines.map(line => {
      ...
      return line.replace(
        /from\s+(['"])(.+?)\1/,
        `from ${quote}${newRelativePath}${quote}`
      )
    })
 ...
 */
function filterImports(imports: string[]): string[] {
 ...
 */
function generateSnapshotComponent(
  extracted: ExtractedComponent,
  snapshotProps: SnapshotProp[]
): string {
  const lines: string[] = []
  ...
  const hookDeclaredVariables = new Set<string>()
  ...
  for (const decl of extracted.variableDeclarations) {
    if (addedDeclarations.has(decl)) continue
...
function generatePropsInterface(
  ...
  for (const prop of snapshotProps) {
    ...
    // FALLBACK: Try to infer type from initial value
      ...
      if (typeof prop.initialValue === 'boolean') type = 'boolean'
      else if (typeof prop.initialValue === 'number') type = 'number'
      else if (typeof prop.initialValue === 'string') type = 'string'
      else if (Array.isArray(prop.initialValue)) type = 'any[]'
      else if (typeof prop.initialValue === 'object' && prop.initialValue !== null) type = 'Record<string, any>'
      else if (typeof prop.initialValue === 'function') type = '(...args: any[]) => any'
    }
