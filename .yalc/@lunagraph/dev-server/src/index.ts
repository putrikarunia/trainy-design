...
import { extractTypesFromFile, type ExtractedPropType } from './extractTypes.js'
import { handleChat, handleChatStream, type ChatMessage, type ChatContext } from './aiChat.js'
 ...
 */
function extractClassesFromElements(elements: FEElement[], classes: Set<string>) {
  function extractClasses(el: FEElement) {
    ...
    if ('children' in el && Array.isArray(el.children)) {
      el.children.forEach(child => extractClasses(child as FEElement))
    }
  }
...
async function generatePagesManifest() {
  ...
  const pages: Array<{ id: string; name: string; elements: any[] }> = []
...
app.post('/api/ai/chat', async (req, res) => {
  try {
    ...
    const { messages, context, stream } = req.body as {
...
app.post('/api/files/*', async (req, res) => {
  try {
    ...
    const { elements, stateContext, internalComponent } = req.body as {
      elements: FEElement[]
      stateContext?: Record<string, any>  // Mock values used during editing
      internalComponent?: {               // For updating only an internal component
        name: string
        startLine: number
        endLine: number
      }
    ...
    const snapshotsToUpdate: string[] = []
...
app.post('/api/snapshot/create', async (req, res) => {
  try {
    const { componentPath, componentName: requestedComponentName } = req.body as {
      componentPath: string  // e.g., "components/ProductList.tsx"
      componentName?: string // e.g., "DropdownMenuTrigger" - the specific component to extract
    }
    ...
    const initialMockValues: Record<string, any> = {}
...
async function updateSnapshotsIndex(snapshotDir: string) {
  try {
    ...
    const snapshotsWithInternals: string[] = []
...
function generateDefaultMockValue(prop: SnapshotProp): any {
  ...
  // Based on source
  if (source === 'useContext') return undefined
  if (source === 'customHook') return undefined
...
async function createCanvasVersion(canvasDir: string, canvasFile: string): Promise<string | null> {
  try {
...
async function pruneCanvasVersions(versionsDir: string): Promise<void> {
  try {
...
async function processElementImages(element: FEElement): Promise<FEElement> {
  ...
  if (element.type === 'html' && element.children && Array.isArray(element.children)) {
    return {
      ...
      children: await Promise.all(
        element.children.map((child: FEElement) => processElementImages(child))
      )
    }
  }
...
app.post('/api/canvas/save', async (req, res) => {
  try {
    const { id, name, elements, zoom, pan, metadata, forceEmpty } = req.body as {
      id?: string
      name: string
      elements: FEElement[]
      zoom?: number
      ...
      forceEmpty?: boolean  // Allow saving empty canvas explicitly
    }
    ...
    const processedElements = await Promise.all(
      elements.map(element => processElementImages(element))
    )
...
app.post('/api/component/create', async (req, res) => {
  try {
    const { sourceFilePath, componentName, code } = req.body as {
...
async function createDraftVersion(draftsDir: string, draftPath: string): Promise<string | null> {
  try {
...
async function pruneDraftVersions(versionsDir: string): Promise<void> {
  try {
    const files = await readdir(versionsDir)
    const versionFiles = files
...
app.post('/api/draft/save', async (req, res) => {
  try {
    const { componentName, filePath, elements, mockValues } = req.body as {
      componentName: string
      filePath: string
      elements: FEElement[]
    ...
    const validation = validatePath(filePath)
    let sourceFileModifiedAt: string | null = null
...
app.get('/api/drafts', async (req, res) => {
  try {
    ...
    const drafts: Array<{ componentName: string; filePath: string; savedAt: string }> = []
