 ...
 */
function findComponentFunction(sourceFile: any): any {
 ...
 */
function extractPropsFromFunction(func: any): ExtractedPropType[] {
  const props: ExtractedPropType[] = []
 ...
 */
function extractUseStateCalls(func: any): ExtractedPropType[] {
  const stateVars: ExtractedPropType[] = []
  const body = func.getBody()
  if (!body) return stateVars
  ...
  for (const varDecl of varDeclarations) {
    ...
    if (!Node.isIdentifier(expression) || expression.getText() !== 'useState') continue
    ...
    if (!Node.isBindingElement(firstElement)) continue
    ...
    if (args.length > 0) {
      ...
      if (argText === 'true') initialValue = true
      else if (argText === 'false') initialValue = false
      else if (argText === 'null') initialValue = null
      else if (/^['"].*['"]$/.test(argText)) initialValue = argText.slice(1, -1)
      else if (/^\d+$/.test(argText)) initialValue = parseInt(argText, 10)
      else if (argText.startsWith('[')) initialValue = [] // Array
      else if (argText.startsWith('{')) initialValue = {} // Object
    }
 ...
 */
function extractInternalConstants(func: any): ExtractedPropType[] {
  const constants: ExtractedPropType[] = []
  const body = func.getBody()
