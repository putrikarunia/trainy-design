 ...
 */
export async function mergeWithClaude(options) {
    const { originalCode, generatedJSX, filePath, componentImports, stateContext, internalComponentName, internalComponentRange } = options;
    let importsSection = '';
...
function cleanClaudeResponse(response) {
    ...
    const looksLikeCode = cleaned.includes('import ') ||
