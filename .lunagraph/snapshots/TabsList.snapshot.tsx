...
export default function TabsListSnapshot({
  __onRender,
  className,
  variant = "default",
  props,
  ...
  const __element = (
    <TabsPrimitive.List data-slot="tabs-list" data-variant={variant} className={cn(tabsListVariants({
  variant
}), className)} {...props} />
  )
  ...
  const __propsKey = React.useMemo(() => {
    try {
      return JSON.stringify({ className, variant, props }, (key, value) => {
