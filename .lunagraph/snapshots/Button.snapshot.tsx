...
export default function ButtonSnapshot({
  ...
  const __element = (
    <Comp data-slot="button" data-variant={variant} data-size={size} className={cn(buttonVariants({
  variant,
  size,
  className
}))} {...props} />
  ...
  const __propsKey = React.useMemo(() => {
    try {
      return JSON.stringify({ className, variant, size, asChild, props }, (key, value) => {
